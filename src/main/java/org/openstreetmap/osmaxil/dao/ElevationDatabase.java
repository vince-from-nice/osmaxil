...
public class ElevationDatabase implements ElevationDataSource {
	...
	/**
	 * Find all points which intersect the including geometry and disjoint the
	 * excluding geometry (useful for multipolygon buildings with "hole"). That
	 * method use a radius as argument in order to skrink the including and
	...
	public List<ElevationImport> findAllElevationsByGeometry(String includingGeomAsWKT, String excludingGeomAsWKT,
		...
		if (excludingGeomAsWKT != null) {
			...
			condition += " AND ST_Disjoint(geom, ST_Transform(ST_Buffer(" + excludingGeom + ", -" + shrinkRadius + "), "
					+ srid + "))";
		}
		...
		List<ElevationImport> results = this.jdbcTemplate.query(query, new RowMapper<ElevationImport>() {
			public ElevationImport mapRow(ResultSet rs, int rowNum) throws SQLException {
				ElevationImport elevation = new ElevationImport(Double.parseDouble(rs.getString("x")),
						Double.parseDouble(rs.getString("y")), Double.parseDouble(rs.getString("z")) * valueScale);
				return elevation;
			}
		});
		return results;
	 ...
	 * Find all points which intersect the including geometry and disjoint the
	 * excluding geometry (useful for multipolygon buildings with "hole"). That
	 * method use a factor as argument in order to scale the including and excluding
	 * geometries.
	 * 
	 * TODO: use JTS instead of PostGis functions (ST_Scale and ST_Translate) to
	 * perform the scale/skrink process because this implementation is working but
	 * it takes 15 minutes to execute !
	 */
	public List<Coordinates> findAllPointsByGeometry(String includingGeomAsWKT, String excludingGeomAsWKT,
			float scaleFactor, int geomSrid) {
		...
		condition = "ST_Translate(" + condition + ", " + "-" + scaleFactor
				+ "*(ST_Xmin(includingGeom)+ST_XMax(includingGeom))/2 + ((ST_Xmin(includingGeom)+ST_XMax(includingGeom))/2), "
				+ "-" + scaleFactor
