...
public class BuildingElevationScorer extends AbstractElementScorer<BuildingElement> {
	...
	public float computeElementMatchingScore(BuildingElement element, float minMatchingScore) {
		...
		int altitude = (int) Math.round(this.dtmDataSource.findElevationByCoordinates(Double.parseDouble(center.x), Double.parseDouble(center.y), this.osmPostgis.getSrid()));
